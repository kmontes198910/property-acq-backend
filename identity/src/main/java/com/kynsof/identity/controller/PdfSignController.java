...
public class PdfSignController {
   private final ServicioFirmaDigital servicioFirmaDigital;
    ...
    public PdfSignController(ServicioFirmaDigital servicioFirmaDigital) {
...
//    }
    ...
    @PostMapping("/firmar")
    public ResponseEntity<?> firmarDocumento(@RequestBody SolicitudFirma solicitud) {
        try {
            servicioFirmaDigital.firmarPDF(
                    solicitud.getRutaP12(),
                    solicitud.getPassword(),
                    solicitud.getRutaPdfEntrada(),
                    solicitud.getRutaPdfSalida(),
                    solicitud.getRectangulo()
            );
            return ResponseEntity.ok("Documento firmado exitosamente");
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error al firmar: " + e.getMessage());
        }
    }
    }