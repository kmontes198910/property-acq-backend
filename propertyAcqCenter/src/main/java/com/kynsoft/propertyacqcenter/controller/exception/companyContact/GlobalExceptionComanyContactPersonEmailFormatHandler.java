package com.kynsoft.propertyacqcenter.controller.exception.companyContact;

import com.kynsoft.propertyacqcenter.application.response.ErrorResponse;
import com.kynsoft.propertyacqcenter.domain.dto.exception.companyContact.PersonEmailFormatException;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.ExceptionHandler;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestControllerAdvice;

import java.time.LocalDateTime;

@RestControllerAdvice
public class GlobalExceptionComanyContactPersonEmailFormatHandler {

    @ExceptionHandler(PersonEmailFormatException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ErrorResponse handleEmergencyCaseNotFound(PersonEmailFormatException ex) {
        return new ErrorResponse(
                HttpStatus.NOT_FOUND.value(),
                "Invalid person email format.",
                ex.getMessage(),
                LocalDateTime.now().toString()
        );
    }
}
