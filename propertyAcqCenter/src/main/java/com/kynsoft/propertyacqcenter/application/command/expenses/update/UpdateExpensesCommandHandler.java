...
@Component
public class UpdateExpensesCommandHandler implements ICommandHandler<UpdateExpensesCommand> {

    private final IExpensesService expensesService;
    private final IPropertyService propertyService;

    public UpdateExpensesCommandHandler(IExpensesService expensesService, IPropertyService propertyService) {
        this.expensesService = expensesService;
        this.propertyService = propertyService;
    }

    @Override
    public void handle(UpdateExpensesCommand command) {
        PropertyDto property = this.propertyService.getById(command.getProperty());

        this.expensesService.update(ExpensesDto.builder()
                .id(command.getId())
                .property(property)
