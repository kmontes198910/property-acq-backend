...
@Repository
public interface EquipoMedicoReadRepository extends JpaRepository<MedicalTeamEntity, UUID>, JpaSpecificationExecutor<MedicalTeamEntity> {
    
    List<MedicalTeamEntity> findBySurgeryId(UUID surgeryId);
    
    List<MedicalTeamEntity> findByMemberId(UUID integranteId);
    ...
    List<MedicalTeamEntity> findBySurgeryIdAndCargo(@Param("surgeryId") UUID surgeryId, @Param("role") String role);
    
    @Query("SELECT e FROM MedicalTeamEntity e WHERE e.specialtyCode = :specialtyCode AND e.businessId = :businessId")
    List<MedicalTeamEntity> findByEspecialidadAndBusinessId(@Param("specialtyCode") String specialtyCode, @Param("businessId") UUID businessId);
}